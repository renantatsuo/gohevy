...
type RoutineSet struct {
	...
	Type            string    `json:"type"`             // Set classification: "normal", "warmup", "failure", or "drop_set"
...
type WorkoutCountResponse struct {
	Count int `json:"count"` // Total number of workouts logged on the authenticated account
}

// UpdatedWorkout carries the identity of a workout that was created or modified.
// Call GetWorkout with ID to retrieve the full current state of the workout.
type UpdatedWorkout struct {
	ID        string    `json:"id"`         // ID of the workout that was created or updated
	UpdatedAt time.Time `json:"updated_at"` // Timestamp of the update (UTC)
}

// DeletedWorkout carries the identity of a workout that was permanently removed from Hevy.
type DeletedWorkout struct {
	ID        string    `json:"id"`         // ID of the workout that was deleted
	DeletedAt time.Time `json:"deleted_at"` // Timestamp when the workout was deleted (UTC)
}

// WorkoutEvent represents a single change to a workout — either an update or a deletion.
// Exactly one of Updated or Deleted will be non-nil, determined by the Type field.
type WorkoutEvent struct {
	Type    string          `json:"type"`              // Event kind: "updated" (created or modified) or "deleted"
	Updated *UpdatedWorkout `json:"updated,omitempty"` // Non-nil when Type is "updated"; contains the workout ID and update timestamp
	Deleted *DeletedWorkout `json:"deleted,omitempty"` // Non-nil when Type is "deleted"; contains the workout ID and deletion timestamp
}

// PaginatedWorkoutEvents is the response type for GetWorkoutEvents.
// Page is 1-based; iterate by incrementing Page until Page == PageCount.
// Note: workout deletions appear as events even though the library exposes no DeleteWorkout endpoint —
// deletions originate from the Hevy mobile/web app.
type PaginatedWorkoutEvents struct {
	Page      int            `json:"page"`       // Current page number (1-based)
	PageCount int            `json:"page_count"` // Total number of pages available
	Events    []WorkoutEvent `json:"events"`     // Workout change events on the current page
}
...
type ExerciseTemplate struct {
	...
	// Type is the measurement type, determining which Set fields are applicable.
	// Known values (per Hevy API docs): "weight_reps", "weighted_bodyweight",
	// "assisted_bodyweight", "distance_duration", "duration", "reps_only".
...
// ExerciseHistoryEntry represents the sets performed for a specific exercise in a single past workout.
// Used to review progression over time for a given exercise.
type ExerciseHistoryEntry struct {
	WorkoutID   string    `json:"workout_id"`   // ID of the workout in which this exercise was performed
	WorkoutDate time.Time `json:"workout_date"` // Date the workout took place (UTC)
	Sets        []Set     `json:"sets"`         // Sets performed for this exercise in that workout
}
